...
from dsn import DSN,get_seg_loss
...
parser.add_argument('--num_workers', type=int, default=16, help='workers num during training [default: 2]')
...
TRAIN_DATASET = InsSegDataset(cfgs.dataset_root, camera=cfgs.camera, split='train',
                                num_points=cfgs.num_point, remove_outlier=True, augment=True)
TEST_DATASET = InsSegDataset(cfgs.dataset_root, camera=cfgs.camera, split='test_seen',
...
#freeze stage1
# params_list=[]
# params_list.append(dict(params=net.view_estimator.parameters(), lr=cfgs.learning_rate))
...
def train_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):
        ...
        loss, end_points = get_seg_loss(end_points)
        loss.backward()
        if (batch_idx + 1) % 1 == 0:
            optimizer.step()
            optimizer.zero_grad()
